...
PRON_NUM_VERB = {"אותו","עצמו","עצמה","עצמם","אחד","אחת","שניים","שתיים","שני","שלוש","ארבע","חמש",
                 ...
                 "היה","הייתה","נהיה","יכול","צריך","רוצה","יודע","רואה","צייר","קשר","לימד","לימדה","מלמד",
                 "למד","למדה","מדבר","דיבר","היה","נהיה","מגיע","מגיעה","עולה","עולה","יורד","יורדת"}
...
def clean_domain_terms(content_text, n=8, min_count=3):
    ...
    for t, c in terms.most_common(40):
        ...
        if t in BLOCKED or _base(t) in BLOCKED:
            continue
        if _base(t) in STOP_HE or _base(t) in TELL_PARTICLES:
            continue
        out.append((t, c))
        if len(out) >= n:
            break
